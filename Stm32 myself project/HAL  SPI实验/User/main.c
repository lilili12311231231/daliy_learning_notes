...
int main(void)
{
    ...
    usmart_dev.init(84);/* 初始化USART1设备 */
    led_init();/* 初始化LED */
    lcd_init();/* 初始化LCD */
    key_init();/* 初始化按键 */
    ...
    id = norflash_read_id();/* 读取NORFLASH ID */
    ...
    lcd_show_string(30, 170, 200, 16, 16, "SPI FLASH Ready!", BLUE);
    ...
    while(1)
    {
        ...
        if(key == KEY1_PRES)/* KEY1按下,写入 */
        {
            ...
            norflash_write((uint8_t *)datatemp, flashsize - 100, TEXT_SIZE);      /* 从倒数第100个地址处开始,写入TEXT_SIZE个字节 */
        ...
        if(key == KEY0_PRES)/* KEY0按下,读取字符串并显示 */
        {
            ...
            norflash_read(datatemp, flashsize - 100, TEXT_SIZE);                   /* 从倒数第100个地址处开始,读取TEXT_SIZE个字节 */
