 ...
 * @version     V1.0
 * @date        2021-10-14
 * @brief       系统初始化相关(包括时钟配置/中断管理/GPIO设置等)
...
{
    /* 设置NVIC中断向量表偏移寄存器, VTOR第9位保留, 保留[8:0] */
    SCB->VTOR = baseaddr | (offset & (uint32_t)0xFFFFFE00);
}

/**
 * @brief       执行 WFI指令(执行此指令后进入休眠状态, 等待中断唤醒)
 * @param       无
 * @retval      无
 */
 ...
 * @param       pllm: 主PLL和音频PLL分频PLL预分频系数(进PLL之前的分频), 取值范围: 2~63.
 * @param       pllp: 主PLL的p分频系数(PLL之后的分频), 分频后作为系统时钟, 取值范围: 2, 4, 6, 8.(只有这4个值)
 * @param       pllq: 主PLL的q分频系数(PLL之后的分频), 取值范围: 2~15.
 * @note
 *
 *              Fvco: VCO频率
 *              Fsys: 系统时钟频率, 也是PLL的p分频后的时钟频率
 *              Fq:   主PLL的q分频后的时钟频率
...
uint8_t sys_stm32_clock_init(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
    ...
    __HAL_PWR_VOLTAGESCALING_CONFIG(PWR_REGULATOR_VOLTAGE_SCALE1);      /* 设置调压器输出电压级别,以便在器件为较高频率工作时 */

    /* 使能HSE, 并选择HSE作为PLL时钟源, 同时开启PLL1并给USB时钟 */
    ...
    /* 选择PLL作为系统时钟源, 并且配置HCLK,PCLK1和PCLK2 */
...
/**
 * @brief       当编译报错时报告出错所在的文件名和函数名, 文件编号以及所在行号
 * @param       file: 指向源文件
 *              line: 源文件中的行号
 * @retval      无
 */
