...
void atim_timx_pwmin_chy_init(void)
{
    ...
    gpio_init_struct.Alternate = GPIO_TIMX_PWMIN_CHY_GPIO_AF;
...
void atim_timx_pwmin_chy_restart(void)
{
     ...
     __HAL_TIM_ENABLE_IT(&g_timx_pwmin_chy_handle); /* 重置通道1 输入捕获中断 */
...
static void atim_timx_pwmin_chy_process(void)
{
    ...
    if(__HAL_TIM_GET_FLAG(&g_timx_pwmin_chy_handle, TIM_FLAG_CC1)) /* 通道1 输入捕获中断标志位 */
    {
        g_timxch_pwmin_hval = HAL_TIM_ReadCapturedValue(&g_timx_pwmin_chy_handle, TIM_CHANNEL_2) + 1; /* 高定平脉宽捕获值  */
        g_timxch_pwmin_lval = HAL_TIM_ReadCapturedValue(&g_timx_pwmin_chy_handle, TIM_CHANNEL_1) + 1; /* 低定平脉宽捕获值,并加1 */
